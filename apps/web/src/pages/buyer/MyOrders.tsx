...
const TABS = [
  { label: "Active", statuses: ["pending", "accepted", "processing", "ready_for_pickup", "out_for_delivery"] },
  { label: "Completed", statuses: ["delivered", "completed"] },
  { label: "Cancelled", statuses: ["cancelled_by_buyer", "cancelled_by_seller", "cancelled_by_admin"] },
];

const STATUS_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  pending: { bg: "rgba(251,191,36,0.12)", text: "#fbbf24", label: "Pending" },
  accepted: { bg: "rgba(96,165,250,0.12)", text: "#60a5fa", label: "Accepted" },
  processing: { bg: "rgba(167,139,250,0.12)", text: "#a78bfa", label: "Processing" },
  ready_for_pickup: { bg: "rgba(52,211,153,0.12)", text: "#34d399", label: "Ready for Pickup" },
  out_for_delivery: { bg: "rgba(96,165,250,0.12)", text: "#60a5fa", label: "Out for Delivery" },
  delivered: { bg: "rgba(74,222,128,0.12)", text: "#4ade80", label: "Delivered" },
  completed: { bg: "rgba(74,222,128,0.12)", text: "#4ade80", label: "Completed" },
  cancelled_by_buyer: { bg: "rgba(248,113,113,0.12)", text: "#f87171", label: "Cancelled" },
  cancelled_by_seller: { bg: "rgba(248,113,113,0.12)", text: "#f87171", label: "Cancelled by Seller" },
  cancelled_by_admin: { bg: "rgba(248,113,113,0.12)", text: "#f87171", label: "Cancelled by Admin" },
  rejected: { bg: "rgba(248,113,113,0.12)", text: "#f87171", label: "Rejected" },
};
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {isLoading ? (
          ...
          >
            ...
            <p style={{ color: "var(--text-soft)" }}>No {TABS[tab].label.toLowerCase()} orders yet.</p>
            {tab === 0 && (
              <Link
                to="/"
